...
const FALLBACK_SERVICES = [
    { id: '1', title: 'Study Abroad', icon: 'school', description: 'Comprehensive guidance from university selection to application and enrollment at top global institutions in Australia, UK, USA, and Canada.' },
    { id: '2', title: 'Migration Services', icon: 'public', description: 'Expert legal pathways for permanent residency, work permits, and family visas. Our certified agents ensure your application is watertight.' },
    { id: '3', title: 'Visa Assistance', icon: 'description', description: 'High-success rate documentation support and rigorous interview preparation sessions to maximize your chances of approval.' },
    { id: '4', title: 'Test Preparation', icon: 'quiz', description: 'Expert coaching for IELTS, TOEFL, PTE, GRE, GMAT, and SAT with proven score improvement strategies.' },
    { id: '5', title: 'Career Counseling', icon: 'work', description: 'Personalized career guidance to align your education with long-term professional goals worldwide.' },
    { id: '6', title: 'Scholarship Guidance', icon: 'emoji_events', description: 'Identify and apply for scholarships to reduce your financial burden and study at top institutions.' },
]
...
const HERO_IMAGES = [
    ...
    // Row 2
    { src: 'https://images.unsplash.com/photo-1498243691581-b145c3f54a5a?w=600', alt: 'Library', cls: 'top-[32%] left-[2%]', w: 110, h: 130, delay: '0.4s', dur: '5.5s', vanishDelay: '2s' },
    { src: 'https://images.unsplash.com/photo-1427504494785-3a9ca7044f45?w=600', alt: 'Students Abroad', cls: 'top-[28%] left-[26%]', w: 150, h: 110, delay: '1.2s', dur: '6.5s', vanishDelay: '5s' },
    { src: 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=600', alt: 'Campus View', cls: 'top-[35%] right-[24%]', w: 100, h: 140, delay: '2s', dur: '4s', vanishDelay: '8s' },
    { src: 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=600', alt: 'Lecture Hall', cls: 'top-[30%] right-0', w: 130, h: 100, delay: '1.8s', dur: '5.2s', vanishDelay: '1s' },
    // Row 3
    { src: 'https://images.unsplash.com/photo-1509062522246-3755977927d7?w=600', alt: 'Group Study', cls: 'bottom-[8%] left-0', w: 130, h: 120, delay: '0.6s', dur: '4.8s', vanishDelay: '4s' },
    { src: 'https://images.unsplash.com/photo-1564981797816-1043664bf78d?w=600', alt: 'Team Project', cls: 'bottom-[2%] left-[28%]', w: 100, h: 90, delay: '1s', dur: '5.8s', vanishDelay: '7s' },
    { src: 'https://images.unsplash.com/photo-1532012197267-da84d127e765?w=600', alt: 'Classroom', cls: 'bottom-[5%] left-[52%]', w: 150, h: 130, delay: '0.2s', dur: '6.2s', vanishDelay: '10s' },
    { src: 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=600', alt: 'High School', cls: 'bottom-[0%] right-0', w: 110, h: 100, delay: '1.4s', dur: '4.6s', vanishDelay: '2.5s' },
    // Floating extras (overlap between rows)
    { src: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=600', alt: 'University Life', cls: 'top-[16%] left-[15%]', w: 80, h: 80, delay: '0.9s', dur: '5.3s', vanishDelay: '5.5s' },
    { src: 'https://images.unsplash.com/photo-1472214103451-9374bd1c798e?w=600', alt: 'Study Time', cls: 'top-[50%] right-[12%]', w: 90, h: 85, delay: '1.6s', dur: '4.2s', vanishDelay: '8.5s' },
]
        ...
        async function fetchData() {
            try {
                const [sRes, cRes, tRes, stRes] = await Promise.all([
                    ...
                    supabase.from('countries').select('*').eq('is_active', true).order('display_order'),
                ...
                if (stRes.data?.length) {
                    const settingsMap = {}
                    stRes.data.forEach(s => { settingsMap[s.key] = s.value })
        ...
        <div ref={containerRef}>
            ...
            {lightbox !== null && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={() => setLightbox(null)}>
                    ...
                    <div className="relative z-10 max-w-[90vw] max-h-[90vh] animate-popIn" onClick={e => e.stopPropagation()}>
                        <img
                            src={HERO_IMAGES[lightbox].src}
                            alt={HERO_IMAGES[lightbox].alt}
                            className="max-w-full max-h-[85vh] object-contain rounded-2xl shadow-2xl"
                        />
                        <p className="text-center text-white text-lg font-bold mt-4">{HERO_IMAGES[lightbox].alt}</p>
                        {/* Close button */}
                        <button
                        ...
                        {/* Prev / Next */}
                        {lightbox > 0 && (
                            <button onClick={() => setLightbox(lightbox - 1)} className="absolute left-[-50px] top-1/2 -translate-y-1/2 w-10 h-10 bg-white/20 backdrop-blur text-white rounded-full flex items-center justify-center hover:bg-white/40 transition-colors">
                                <span className="material-symbols-outlined">chevron_left</span>
                            </button>
                        )}
                        {lightbox < HERO_IMAGES.length - 1 && (
                            <button onClick={() => setLightbox(lightbox + 1)} className="absolute right-[-50px] top-1/2 -translate-y-1/2 w-10 h-10 bg-white/20 backdrop-blur text-white rounded-full flex items-center justify-center hover:bg-white/40 transition-colors">
                    ...
                    <div className="grid lg:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="max-w-2xl opacity-0 animate-fadeInUp">
                            ...
                            <p className="text-lg text-gray-600 dark:text-gray-400 mb-10 leading-relaxed font-light animate-fadeInUp animate-delay-100">
                                Your bridge to {stats.partner_universities} world-class universities and seamless
                                migration pathways. Join {stats.students_placed} successful students achieving
                                their dreams abroad.
                            </p>
                            ...
                            <div className="mt-12 flex items-center gap-6 animate-fadeInUp animate-delay-300">
                                ...
                                <div className="text-sm">
                                    <p className="font-bold text-[#0d121b] dark:text-white">Join {stats.students_placed} Alumni</p>
                        ...
                        <div className="relative opacity-0 animate-slideInRight animate-delay-200">
                            ...
                            <div className="relative h-[420px] sm:h-[480px] lg:h-[540px] w-full">
                                {HERO_IMAGES.map((img, i) => (
                                    ...
                                    >
                                        <img src={img.src} alt={img.alt} className="w-full h-full object-cover transition-transform duration-500" loading="lazy" />
                                        ...
                                        <div className="absolute bottom-3 left-3 right-3 text-white opacity-0 translate-y-2 transition-all duration-300 mosaic-label">
                                            <p className="text-xs font-bold drop-shadow-lg">{img.alt}</p>
                                        </div>
                                    </div>
                                ))}

                                {/* Floating stats badge */}
                                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md px-6 py-3 rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 flex items-center gap-4" style={{ animation: 'heroFloat 4s ease-in-out infinite 0.5s' }}>
                                    <div className="text-center">
                                        <p className="text-xl font-black text-primary">{stats.countries_count || '50+'}</p>
                                        <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Countries</p>
                                    </div>
                                    <div className="w-px h-8 bg-gray-200 dark:bg-gray-700" />
                                    <div className="text-center">
                                        <p className="text-xl font-black text-primary">{stats.partner_universities}</p>
                                        <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Universities</p>
                                    </div>
                                    <div className="w-px h-8 bg-gray-200 dark:bg-gray-700" />
                                    <div className="text-center">
                                        <p className="text-xl font-black text-primary">{stats.visa_success_rate}</p>
                                        <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Success</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <section className="bg-primary py-12 relative overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                        {[
                            { val: stats.partner_universities, label: 'Partner Universities' },
                            ...
                            >
                                <p className="text-4xl lg:text-5xl font-black mb-1">{stat.val}</p>
            ...
            <section className="py-24 bg-background-light dark:bg-background-dark/50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center max-w-3xl mx-auto mb-16 reveal">
                        <h2 className="text-primary font-bold text-sm uppercase tracking-widest mb-3">Our Expert Services</h2>
                        ...
                        <p className="text-gray-600 dark:text-gray-400 text-lg">
                            We provide end-to-end support for your education and migration journey, ensuring a smooth transition to your new life abroad.
                        </p>
                    </div>
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {services.slice(0, 6).map((service, i) => (
                            ...
                            >
                                ...
                                <h4 className="text-2xl font-bold mb-4 text-[#0d121b] dark:text-white">{service.title}</h4>
            ...
            <section className="py-24 bg-white dark:bg-background-dark">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center max-w-3xl mx-auto mb-16 reveal">
                        <h2 className="text-primary font-bold text-sm uppercase tracking-widest mb-3">Study Destinations</h2>
                        <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#0d121b] dark:text-white mb-6">
                            Explore Top Countries
                        </h3>
                        <p className="text-gray-600 dark:text-gray-400 text-lg">
                            Discover the best study destinations in the world and find the perfect fit for your academic journey.
                        </p>
                    </div>
                        ...
                        {countries.slice(0, 6).map((country, i) => (
                            ...
                            >
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-6 text-white z-10">
                                    ...
                                    <p className="country-info text-sm text-white/80 mt-1">{country.description}</p>
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            <section className="py-24 bg-background-light dark:bg-background-dark/50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center max-w-3xl mx-auto mb-16 reveal">
                        <h2 className="text-primary font-bold text-sm uppercase tracking-widest mb-3">Success Stories</h2>
                        <h3 className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#0d121b] dark:text-white mb-6">
                        ...
                        {testimonials.map((t, i) => (
                            ...
                            >
                                <div className="flex items-center gap-1 mb-4">
                                    {[...Array(5)].map((_, j) => (
                                        <span key={j} className="material-symbols-outlined text-yellow-400 text-lg" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                                    ))}
                                </div>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    />
                                    <div>
                                        <p className="font-bold text-sm text-[#0d121b] dark:text-white">{t.student_name}</p>
                                        <p className="text-xs text-gray-500">{t.university}, {t.country}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                        ...
                        <div className="relative z-10 grid lg:grid-cols-2 gap-12 items-center">
                            <div className="reveal">
                                ...
                                <p className="text-gray-400 text-lg mb-10">
                                    Our expert counselors are waiting to help you choose the right path for your academic and professional growth.
                                </p>
